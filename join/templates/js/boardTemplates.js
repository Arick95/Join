...
function renderStatusfieldsTemplateHTML(i, stat, statClass) {
    ...
    <div class="statContainer">
        ...
        <div id="statContainer${i}" class="showContainer" ondrop="drop('${statClass}'); stopHighlight('statContainer${i}')" ondragover="allowDrop(event); highlight('statContainer${i}')" ondragleave="stopHighlight('statContainer${i}')"></div>
    </div>
`
}
/**
...
function renderCheckedBoxTemplateHTML(i, Id, subtask) {
    return /*html*/`
    <div id="subtaskBox${i}" class="subtask modifySubtask">
        <input id="subtaskCheckBox${i}" type="checkbox" checked onclick="configDoneSubtask(${i}, ${Id})">
        <p id="subtaskName${i}">${subtask}</p>
        <button class="btn-subTaskDeletet" onclick="deletetSubTasksFromModyfier('${i}','${Id}','${subtask}')">x</button>
    </div>`
}
...
function renderCheckedBoxPopup(i, Id, subtask) {
    return /*html*/`
    <div id="subtaskBox${i}" class="subtask modifySubtask">
        <input id="subtaskCheckBox${i}" type="checkbox" checked onclick="configDoneSubtaskPopup(${i}, ${Id})">
        <p id="subtaskName${i}">${subtask}</p>
    </div>`
}

function renderUncheckedBoxPopup(i, Id, subtask) {
    return /*html*/`
    <div id="subtaskBox${i}" class="subtask modifySubtask">
        <input id="subtaskCheckBox${i}" type="checkbox"  onclick="configDoneSubtaskPopup(${i}, ${Id})">
        <p id="subtaskName${i}">${subtask}</p>
    </div>
    `
}
...
function deletetSubTasksFromModyfier(i, Id, subtask) {
   
    document.getElementById(`subtaskBox${i}`).classList.add('d-none');
    addNumbertoBoard(i, Id);  
}
