...
function renderContactsAddTask(Id) {
    ...
    for (let i = 0; i < allContacts.length; i++) {
        ...
        <div class="assignedFrame" id="assignedFrame${i}" onclick="addingAssignedByName(${i})">
        <input id="assignedCheckbox${i}" onclick="renderAssignedInitial(),stopingProgagation()" class="assignedCheckbox" type="checkbox">
        <div>${name}</div></div>
        `;
    }
...
function deletetSubtaskFormTaskCreater(i) {
...
function renderSubtask(){
    ...
    for (let i = 0; i < allSubtasks.length; i++) {
        ...
        <div class="subtask" >
            <div class="subtask">
                <input id="subtask(${i})" type="checkbox">
                <p>${subtask}</p>
            </div>
            <button class="btn-subTaskDeletet" type="button" onclick="deletetSubtaskFormTaskCreater(${i})">
            x
            </button>
        </div>
    `;
...
function newSubtaskEditOverlay() {
    ...
    <div class="subtask">
        ...
        <button class="btn-subTaskDeletet" onclick="deleteNewCreateSubTaskFromTaskEditor()">x</button>
    </div>
    ` ;
    ...
    renderModifySubtaskList(currentOpenTask);
}

function deleteNewCreateSubTaskFromTaskEditor(){
    
    let subtask = newTaskArray[currentOpenTask]['subtasks']
