...
let readytoCheckedSave = true;
let saveTheOldSubtask = [];

/**
 * 
 * Adding a id System for each Task importend to editer the Task and Drag and Drop function.
 */
function giveTaskId() {
...
function renderClickedTaskPopUpHTML(Id) {
    ...
    renderSubtaskListOnPopup(Id);
}

function renderSubtaskListOnPopup(Id) {
    let content = document.getElementById('PopupSubTask');
    content.innerHTML = '';
    ...
    for (let i = 0; i < task['subtasks'].length; i++) {
        ...
        if (isChecked == true) {
            content.innerHTML += renderCheckedBoxPopup(i, Id, subtask);

        }
        if (isChecked == false) {
            content.innerHTML += renderUncheckedBoxPopup(i, Id, subtask);
        }
    }
}
